...
// Landing de captación pura (SEO / AEO / GEO), oculta del menú de
// navegación: el objetivo es posicionar en buscadores y motores de
// respuesta de IA, y convertir directamente en la misma página — el
// tarificador completo y la calculadora de ahorro están embebidos aquí, sin
// enlazar a ninguna otra URL para completar el proceso. Reutiliza el
// contenido y componentes ya existentes de /seguro-de-salud (beneficios,
// coberturas, confianza, testimonios) en vez de duplicarlos.
export async function SeoLandingPage({ page, precio }: { page: SeoLandingPageData; precio: number }) {
  const base = getProductPage("salud")!;
  const theme = await getTheme();
  const heroImage = theme.heroImages.salud;
  const faq = getCombinedFaq(page);
  const otras = SEO_LANDING_PAGES.filter((p) => p.slug !== page.slug);
  const quickAnswer = page.quickAnswerTemplate.replace("{precio}", priceLine(precio).toLowerCase());

  const localTestimonial = page.geo
    ? EXAMPLE_TESTIMONIALS.find((t) => t.place.includes(page.geo!.islandName.split(" ")[0]))
    : undefined;
  ...
  const benefits = base.benefits.map((b) =>
    b.icon === "pin"
      ? { ...b, d: page.geo ? `Trabajamos en toda España, con cercanía especial en ${page.geo.islandName}.` : b.d }
      : b
  );
  ...
  return (
    <>
      ...
      <main id="contenido">
        ...
        {/* RESPUESTA RÁPIDA (AEO/GEO) */}
        <section aria-labelledby="respuesta-rapida" className="mx-auto mt-8 max-w-app px-5 md:max-w-5xl lg:max-w-6xl">
          <div className="rounded-card border border-navy/15 bg-navy/[0.04] p-5">
            <h2 id="respuesta-rapida" className="text-[13px] font-bold uppercase tracking-wide text-navy">En resumen</h2>
            <p className="mt-2 text-[15px] leading-relaxed text-ink">{quickAnswer}</p>
          </div>
        </section>

        {/* CALCULADORA DE AHORRO */}
        <section id="calculadora" aria-labelledby="calc-heading" className="mx-auto mt-14 max-w-app px-5 md:mt-24 md:max-w-3xl">
          <SavingsCalculator slug={page.slug} pricePerAsegurado={precio} />
        </section>
        ...
        {page.geo && (
          <section aria-labelledby="local" className="mx-auto mt-14 max-w-app px-5 md:mt-24 md:max-w-5xl lg:max-w-6xl">
            <div className="rounded-[24px] border border-hair bg-white p-6 shadow-soft md:p-10">
              <h2 id="local" className="text-[22px] font-extrabold text-navy md:text-[26px]">Seguro de salud pensado para {page.geo.islandName}</h2>
              <p className="mt-3 text-[14px] leading-relaxed text-slate2">{page.geo.localIntro}</p>
        ...
        {/* TARIFICADOR EMBEBIDO */}
        <section id="tarificador" aria-labelledby="tarificador-heading" className="mx-auto mt-14 max-w-app px-5 md:mt-24 md:max-w-5xl lg:max-w-6xl">
          <div className="rounded-[24px] border border-hair bg-white p-6 shadow-card md:p-10">
            <TarificadorExperience
              variant="salud"
              headingLevel="h2"
              origen="seo-landing"
              heading="Calcula tu precio exacto"
              subheading="Un minuto. Sin compromiso. Ve tu precio real, no una estimación."
            />
          </div>
        </section>
          ...
          <div className="rounded-[24px] bg-navy p-6 text-white md:flex md:items-center md:justify-between md:gap-10 md:p-12">
            <div>
              ...
              <p className="mt-2 text-[15px] leading-relaxed text-white/80 md:text-[16px]">
                Calcula tu precio en 1 minuto o tu ahorro estimado, aquí mismo. Sin compromiso.
              </p>
            </div>
            <div className="mt-5 flex flex-col gap-3 md:mt-0 md:flex-row md:shrink-0">
              ...
              <a href="#calculadora" className="flex items-center justify-center rounded-card border border-white/30 px-5 py-3.5 text-[16px] font-semibold text-white transition-colors hover:bg-white/10">Calcula tu ahorro</a>
            </div>
          </div>
        </section>
        ...
        <section aria-labelledby="seo-heading" className="mx-auto mt-14 max-w-app px-5 pb-4 md:mt-24 md:max-w-2xl">
          ...
          <p className="mt-3 text-[14px] leading-relaxed text-slate2">{page.seoIntro}</p>
        ...
        <section aria-labelledby="tambien" className="mx-auto mt-14 max-w-app px-5 pb-4 md:mt-24 md:max-w-5xl lg:max-w-6xl">
          <h2 id="tambien" className="text-[18px] font-bold text-navy">También te puede interesar</h2>
          <ul className="mt-3 flex flex-wrap gap-2">
            {otras.map((p) => (
              <li key={p.slug}>
                <a href={p.path} className="inline-flex items-center rounded-pill border border-hair bg-white px-3 py-1.5 text-[13px] font-semibold text-navy transition-colors hover:bg-mist">
                  {p.badge}
                </a>
              </li>
            ))}
            <li>
              <a href="/seguro-de-salud" className="inline-flex items-center rounded-pill border border-hair bg-white px-3 py-1.5 text-[13px] font-semibold text-navy transition-colors hover:bg-mist">
                Seguro de salud
              </a>
            </li>
          </ul>
      ...
      <Footer />
      <StickyMobileCta label="Calcula tu precio" href="#tarificador" secondaryLabel="Calcula tu ahorro" secondaryHref="#calculadora" />
    </>
  );
}
